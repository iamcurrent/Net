...
test_set =data[5000:6000,0:1]   # 后300天的开盘价作为测试集
...
def NewStruct(x):
    ...
    for i in range(index):
        ...
        mulmin = np.min(mul)
        mulstd = np.std(mul)
        mulaver = np.average(mul)

        maxs = np.max(x[i])
        mins = np.min(x[i])
        std = np.std(x[i])
        means = np.average(x[i])
        p2p = maxs - mins
        fft = np.fft.fft(x[i])
        fftshift  = np.fft.fftshift(fft)
        fftshift = fftshift[int(len(fftshift)/2):len(fftshift)]
        val = np.abs(fftshift)
        fftmax = np.max(val)
        fftmin = np.min(val)
        fftp2p = fftmax - fftmin
        su = np.sum(val)/len(val)
        res.append(maxs)
        res.append(mins)
        res.append(std)
        res.append(means)
